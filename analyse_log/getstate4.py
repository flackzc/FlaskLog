...
def state_log3():
            ...
            if len(log_list) >= 9:
                ...
                if log_list[9].strip('.') == "MSG_AUTO_CLEAN":
                    ...
                    if get_workstatus_by_time2(log_list[1].split('.')[0]):
                        print(log_list[1], dealmsg(log_list[9].strip('.')),' '.join(get_workstatus_by_time2(log_list[1].split('.')[0])).replace(",",""))
                    else:
                        print(log_list[1], dealmsg(log_list[9].strip('.')),get_workstatus_by_time2(log_list[1].split('.')[0]))
    ...
    # 根据CheckMachineError获取底板报警状态
            ...
            if "dead lock" in i:
                ...
                print(log_list[1], "[INFO] 找到未覆盖区域%s" % ''.join(log_list[9:13]).rstrip(","))
        ...
        # elif "CPathPlan" in i and not ignore(i):
            if "MAStar" in i:
                print(log_list[1], "[INFO] 开始Astar搜路", ''.join(log_list[6:]))

            elif "no path found" in i:
                print(log_list[1], "[INFO] AStar搜路no path found")
            ...
            print(log_list[1], StateChange(i))
            ...
            try:
                ...
                if abs(abs(int(wheelL)) - abs(int(wheelR))) > 25:
                    print(log_list[1],"[error] 轮组差速异常", "左轮：%s,右轮：%s" %(wheelL, wheelR)) 
            ...
            # print(len(log_list))
            print(log_list[1], "[INFO] 单次搜索时间 %sms" % log_list[11].split("=")[1], ''.join(log_list[6:11]).rstrip(","))

        elif "get_closet_cell_run_time" in i and not ignore(i):
            print(log_list[1], "[INFO] 区域搜索时间 %sms" % log_list[7].split("=")[1])
        
        elif "CSlip::Run" in i and not ignore(i):
            print(log_list[1], "[warning] 机器打滑了")
...
if __name__ == "__main__":
    ...
    if sys.argv[1].lower() == "simple" or sys.argv[1].lower() == "s":
        ...
        state_log3()
    elif sys.argv[1].lower() == "odom" or sys.argv[1].lower() == "o":
        getOdom()
